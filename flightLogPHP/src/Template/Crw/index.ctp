 ...
 * @var \App\Model\Entity\Crw[]|\Cake\Collection\CollectionInterface $crw
 */
?>
...
<div class="crw index large-9 medium-8 columns content">
    <h3><?= __('Crw') ?></h3>
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php foreach ($crw as $crw): ?>
            <tr>
                <td><?= h($crw->title) ?></td>
                <td><?= $this->Number->format($crw->id) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('View'), ['action' => 'view', $crw->id]) ?>
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $crw->id]) ?>
                    <?= $this->Form->postLink(__('Delete'), ['action' => 'delete', $crw->id], ['confirm' => __('Are you sure you want to delete # {0}?', $crw->id)]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
